...
import { format, addDays, addHours, subDays, subMonths } from "date-fns";
...
async function seed() {
  try {
    ...
    if (existingUsers.length > 0 && existingTeams.length > 0) {
      const existingUserTeams = await db.query.userTeams.findMany();
      if (existingUserTeams.length === 0) {
        ...
        for (const assignment of userTeamAssignments) {
          await db.insert(schema.userTeams).values(assignment);
        }
      } else {
    ...
    // Create deals
    const stages = await db.query.pipelineStages.findMany({ orderBy: (pipelineStages, { asc }) => [asc(pipelineStages.order)] });
      ...
      if (existingDeals.length === 0) {
        ...
        const dealsData = [
          ...
          },
          {
            ...
            ownerId: users[1].id,
            expectedCloseDate: addDays(new Date(), 45),
            probability: 30,
          ...
          // Qualified stage
          {
            ...
            ownerId: users[0].id,
            expectedCloseDate: addDays(new Date(), 60),
            probability: 40,
          ...
          // Proposal stage
          {
            ...
            ownerId: users[1].id,
            expectedCloseDate: addDays(new Date(), 15),
            probability: 60,
            createdAt: subDays(new Date(), 1),
            updatedAt: subDays(new Date(), 1)
          },
          ...
          // Closed Won stage
          {
            ...
            ownerId: users[0].id,
            expectedCloseDate: subDays(new Date(), 7),
            probability: 100,
        ...
        const activitiesData = [
          {
            ...
            createdAt: subDays(now, 0, 30) // 30 minutes ago
          },
          {
            type: "call",
            ...
            createdAt: subDays(now, 0, 120) // 2 hours ago
          },
          {
            type: "update",
            ...
            createdAt: subDays(now, 1) // Yesterday
          },
          {
            type: "meeting",
            ...
            createdAt: subDays(now, 2) // 2 days ago
          },
          {
            type: "note",
            title: "Note added",
            ...
            createdAt: subDays(now, 3) // 3 days ago
          }
        ];
